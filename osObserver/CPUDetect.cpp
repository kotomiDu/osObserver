...
void CPUDetect::GetCPUVendorString(std::string& cpuVendorString)
{
	int cpuInfo[4] = { -1 };
	char vendor[0x10];
...
void CPUDetect::GetCPUBrandString(std::string& cpuBrandString)
{
	int nExIds;
	int cpuInfo[4] = { -1 };
	char brand[0x40];
...
int CPUDetect::GetCoreInfo(CPUDetect::CPUData::CoreInfo& coreInfo)
{
	LPFN_GLPIex glpi;
	BOOL done = FALSE;
...
int CPUDetect::InitCPUInfo(CPUDetect::CPUData& cpuData)
{
	int returnCode = 0;
...
LPCWSTR ThreadInfo::GetThreadCurrentProcessorStates(CPUDetect::CPUData g_cpuData)
{
...
void ThreadInfo::UpdateThreadInformation() {
